...
def process_hwu64(output_dir):
    assert os.path.exists("NLU-Data-Home-Domain-Annotated-All.csv"), (
        "please download NLU-Data-Home-Domain-Annotated-All.csv as instructed and put it in current directory"
    )
    
    with open("NLU-Data-Home-Domain-Annotated-All.csv", "r") as f:
        raw_data = list(csv.reader(f, delimiter=";"))[1:]  # Считываем данные, пропуская заголовок

    # Создаем словарь метка -> id
    label2id = {}
    current_id = 0

    hwu64 = []
    for item in raw_data:
        # Формируем метку как комбинацию scenario + intent
        raw_label = item[2] + "_" + item[3]
        if raw_label not in label2id:
            label2id[raw_label] = current_id
            current_id += 1
        
        label = label2id[raw_label]
        text = item[9]  # Используем текстовый ответ
        hwu64.append([text, label])

    # Разделяем данные на train/test
    total_count = {i: 0 for i in range(len(label2id))}
    for item in hwu64:
        total_count[item[1]] += 1

    cat_count = {i: 0 for i in range(len(label2id))}
    train = []
    test = []
    for item in hwu64:
        cat = item[1]
    ...
    hwu64_path = os.path.join(output_dir, "intent", "hwu64")
    os.makedirs(hwu64_path, exist_ok=True)
    ...
    # Генерация few-shot данных
    for data_ratio in [1.1, 5]:
        _generate_few_shot_data_files(data_dir=hwu64_path, data_ratio=data_ratio, num_runs=10, task="seq")

    # Сохраняем словарь меток для дальнейшего использования
    with open(os.path.join(hwu64_path, "hwu64_label2id.json"), "w") as f:
