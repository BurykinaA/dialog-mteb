...
class CustomModel(nn.Module):
    ...
    def copy_parameters_from(self, source_model):
        ...
        # Assuming source_model is a CustomModel (student) and self is CustomModel (teacher)
        # Student might have AutoModelForMaskedLM, teacher has AutoModel
        # We need to copy the base model weights.
        
        source_base_model = None
        if hasattr(source_model.model, 'base_model'): # Common for *ForMaskedLM models
            source_base_model = source_model.model.base_model
        elif hasattr(source_model.model, 'bert'): # e.g. BertForMaskedLM
            source_base_model = source_model.model.bert
        elif hasattr(source_model.model, 'roberta'): # e.g. RobertaForMaskedLM
            source_base_model = source_model.model.roberta
        elif hasattr(source_model.model, 'distilbert'): # e.g. DistilBertForMaskedLM
            source_base_model = source_model.model.distilbert
...
class PSCBert(BertPreTrainedModel):
    ...
    def copy_parameters_from(self, source_model):
        ...
        if hasattr(source_model, 'bert') and hasattr(self.bert, 'load_state_dict'):
            ...
            print("Error: Could not copy parameters for PSCBert. Source or target .bert attribute missing or invalid.")
...
class PSCRoberta(RobertaPreTrainedModel):
    ...
    def copy_parameters_from(self, source_model):
        ...
        if hasattr(source_model, 'roberta') and hasattr(self.roberta, 'load_state_dict'):
            ...
            print("Error: Could not copy parameters for PSCRoberta. Source or target .roberta attribute missing or invalid.")
